...
import torch
from .basenet import _BaseNet


def activation_func(activation, is_inplace=False):
    return nn.ModuleDict(
        [["ReLU", nn.ReLU(inplace=is_inplace)], ["None", nn.Identity()]]
    )[activation]
...
def conv_layer(
    filter_size, padding=1, is_batch_norm=False, activation_type="ReLU", *args, **kwargs
):
...
class ResNet(_BaseNet):
    ...
    def __init__(self, in_ch=2, chans=64, num_of_resblocks=5):
        ...
        self.layer2 = ResNetBlocksModule(
            filter_size=filter2, num_blocks=num_of_resblocks
        )
    ...
    def forward(self, input_x):        
        input_x = self.complex_to_chan_dim(input_x)
        ...
        nw_out = self.chan_complex_to_last_dim(nw_out)
